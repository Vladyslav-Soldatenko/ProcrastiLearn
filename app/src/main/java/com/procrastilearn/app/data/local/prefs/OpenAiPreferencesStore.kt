...
class OpenAiPreferencesStore
    ...
    constructor(
        ...
        fun readAiTranslationDirection(): Flow<AiTranslationDirection> =
            ds.data.map { p ->
                val stored = p[K.AI_TRANSLATION_DIRECTION] ?: AiTranslationDirection.FOREIGN_TO_NATIVE.name
                runCatching { AiTranslationDirection.valueOf(stored) }.getOrDefault(AiTranslationDirection.FOREIGN_TO_NATIVE)
            }
