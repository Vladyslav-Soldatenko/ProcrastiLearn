...
class PendingWordRepositoryImplTest {
    ...
    fun `queuePendingWord persists the word and direction`() =
        runTest {
            repository.queuePendingWord("Haus", AiTranslationDirection.TARGET_TO_NATIVE)

            val snapshot = repository.getAllPendingWordsSnapshot()
            assertThat(snapshot).hasSize(1)
            assertThat(snapshot.first().word).isEqualTo("Haus")
            assertThat(snapshot.first().direction).isEqualTo(AiTranslationDirection.TARGET_TO_NATIVE)
        }
    ...
    fun `queuePendingWord replaces existing entry for the same word`() =
        runTest {
            repository.queuePendingWord("Haus", AiTranslationDirection.TARGET_TO_NATIVE)
            repository.queuePendingWord("Haus", AiTranslationDirection.NATIVE_TO_TARGET)

            val snapshot = repository.getAllPendingWordsSnapshot()
            assertThat(snapshot).hasSize(1)
            assertThat(snapshot.first().direction).isEqualTo(AiTranslationDirection.NATIVE_TO_TARGET)
        }
